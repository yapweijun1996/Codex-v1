...
@media (max-width: 720px) {
  ...
  function renderFriendlyPipelineError(error) {
    const raw =
      (error && typeof error === 'object' && 'message' in error && error.message) || String(error || '');
    const message = raw.trim();
    const normalized = message.toLowerCase();

    if (normalized.includes('sql agent could not build a valid query')) {
      return [
        '⚠️ AI 無法產出對應的 SQL 查詢。',
        '',
        '請協助提供更多商業上下文：',
        '- 描述需要分析的資料表與欄位 (e.g. orders, amount)。',
        '- 加上時間範圍或條件 (例如：近30天、狀態為 pending)。',
        '- 若想比對趨勢，請說明比較維度。',
        '',
        '👉 請再提供上述細節，我可以重新規劃查詢 (Need more concrete table/field hints for SQL)。'
      ].join('\n');
    }
        ...
        const handleSend = async () => {
          ...
          try {
            ...
            if (error.name === "AbortError") {
              ...
              const friendlyError = renderFriendlyPipelineError(error);
              appendBubble("assistant", friendlyError);
              state.messages.push({ role: "assistant", content: friendlyError });
              state.conversation.push({ role: "assistant", content: friendlyError });
              debug.log("pipeline:error", error?.message || error);
            }
          } finally {
            resetStreamingState();
