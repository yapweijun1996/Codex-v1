  ...
  const TableResizer=(function(){
    ...
    function createState(inst, options){
      ...
      function ensureCellMetrics(cell){
        if(!cell) return null;
        const existing=cell.__weditorRowMetrics;
        ...
        const metrics={ padTop, padBottom, borderTop, borderBottom, lineHeight, boxSizing };
      ...
      function tightenCellContentSpacing(cell){
        ...
        for(let i=0;i<kids.length;i++){
          const child=kids[i];
          if(!child || child.nodeType!==1 || !child.style) continue;
          let record=child.__weditorRowMargins;
          if(!record){
            let marginTop="";
            let marginBottom="";
            if(win && win.getComputedStyle){
            ...
            record={ marginTop, marginBottom };
            child.__weditorRowMargins=record;
          }
          child.style.marginTop="0px";
          child.style.marginBottom="0px";
        }
        cell.__weditorRowTight=true;
      }
      function restoreCellContentSpacing(cell){
        if(!cell || !cell.children) return;
        const kids=cell.children;
        for(let i=0;i<kids.length;i++){
          const child=kids[i];
          if(!child || child.nodeType!==1 || !child.style) continue;
          const record=child.__weditorRowMargins;
          if(record){
            if(record.marginTop && record.marginTop.trim()) child.style.marginTop=record.marginTop;
            else child.style.removeProperty("margin-top");
            if(record.marginBottom && record.marginBottom.trim()) child.style.marginBottom=record.marginBottom;
            else child.style.removeProperty("margin-bottom");
          } else {
            child.style.removeProperty("margin-top");
            child.style.removeProperty("margin-bottom");
          }
        }
        cell.__weditorRowTight=false;
      }
      ...
      function setRowSize(row, value){
        ...
        for(let i=0;i<cells.length;i++){
          ...
          if(spaceForContent < basePaddingTotal + MIN_HEIGHT){
            ...
            tightenCellContentSpacing(cell);
          } else {
            cell.style.lineHeight=lineHeightValue;
            restoreCellPadding(cell, metrics);
            if(cell.__weditorRowTight) restoreCellContentSpacing(cell);
          }
