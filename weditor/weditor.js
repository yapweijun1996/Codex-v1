(function(){
  ...
  const TableSelection=(function(){
    ...
    function removeCellFromSelection(state, cell){
      if(!state || !cell) return false;
      if(!state.cells.has(cell)) return false;
      state.cells.delete(cell);
      if(cell.classList) cell.classList.remove(CELL_CLASS);
      if(!state.cells.size){
        state.table=null;
      }
      return true;
    ...
    function deleteSelection(state){
      ...
      const validCells=Array.from(state.cells).filter(function(cell){
        return cell && cell.isConnected && state.root.contains(cell);
      });
      if(!validCells.length){
        clearSelection(state);
        return false;
      }
      let table=state.table && state.table.isConnected ? state.table : null;
      if(!table && validCells.length){
        table=tableFromCell(validCells[0]);
      }
    ...
    function handleMouseDown(state, event){
      ...
      if(event.shiftKey){
        ...
        if(state.table && targetTable && state.table!==targetTable){
          clearSelection(state);
        }
        event.preventDefault();
        if(!state.cells.size){
        ...
        if(state.cells.has(cell)){
          removeCellFromSelection(state, cell);
        } else {
