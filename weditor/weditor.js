(function(){
  ...
  const Formatting=(function(){
    ...
    function applyFontFamily(inst, ctx, family){
      ...
      const styled=applyStyleToTableCells(inst, ctx, target, function(cell){
        if(cell.style.fontFamily===family){ return false; }
    ...
    function applyFontSize(inst, ctx, sizeLabel){
      ...
      const styled=applyStyleToTableCells(inst, ctx, target, function(cell){
        if(cell.style.fontSize===meta.px){ return false; }
    ...
    function getTableSelectionCells(inst, ctx, target){
      if(!target || typeof TableSelection==="undefined" || !TableSelection || typeof TableSelection.getSelection!=="function"){
        return null;
      }
      ...
      for(let i=0;i<selection.cells.length;i++){
        ...
        if(cell && cell.nodeType===1 && cell.isConnected && target.contains(cell)){
          cells.push(cell);
        }
      }
      return cells.length?cells:null;
    }
    function applyCommandToTableCells(inst, ctx, target, handler){
      const cells=getTableSelectionCells(inst, ctx, target);
      ...
      for(let i=0;i<sel.rangeCount;i++){
        try{ originalRanges.push(sel.getRangeAt(i).cloneRange()); }
        catch(err){}
      }
      let changed=false;
      for(let i=0;i<cells.length;i++){
        const cell=cells[i];
        if(!cell || !cell.isConnected || !target.contains(cell)) continue;
        const range=doc.createRange();
        range.selectNodeContents(cell);
        sel.removeAllRanges();
        sel.addRange(range);
        try{
          if(handler(cell, range, sel)){ changed=true; }
        }catch(err){ }
      }
      sel.removeAllRanges();
      ...
      TableSelection.setSelection(inst, ctx, cells);
      return changed;
    }
    function applyStyleToTableCells(inst, ctx, target, updater){
      const cells=getTableSelectionCells(inst, ctx, target);
      if(!cells){ return false; }
      let changed=false;
      for(let i=0;i<cells.length;i++){
        const cell=cells[i];
        if(!cell || cell.nodeType!==1 || !cell.style) continue;
        try{
          if(updater(cell)===true){ changed=true; }
        }catch(err){ }
        if(cell.getAttribute && cell.getAttribute("style") && !cell.getAttribute("style").trim()){
          cell.removeAttribute("style");
        }
      }
      if(changed){ TableSelection.setSelection(inst, ctx, cells); }
      return changed;
    }
    ...
    function applyShading(inst, ctx, color){
      ...
      const normalized=color?normalizeShadingColor(color):null;
      let changed=false;
      const shadingCells=getTableSelectionCells(inst, ctx, target);
      if(shadingCells){
        changed=applyShadingToElements(shadingCells, normalized);
        if(changed){ TableSelection.setSelection(inst, ctx, shadingCells); }
      }
      if(!changed){
    ...
    function applyFontColor(inst, ctx, color){
      ...
      const styled=applyStyleToTableCells(inst, ctx, target, function(cell){
        ...
        if(cell.style.color===color){ return false; }
        cell.style.color=color;
        return true;
      });
      if(styled){ if(inst){ inst.fontColor=color; } return true; }
    ...
    function clearFontColor(inst, ctx){
      const target=resolveTarget(inst, ctx); if(!target) return false;
      const cleared=applyStyleToTableCells(inst, ctx, target, function(cell){
        let modified=false;
        if(cell.style && cell.style.color){
          cell.style.removeProperty("color");
          modified=true;
        }
        if(cell.hasAttribute && cell.hasAttribute("color")){ cell.removeAttribute("color"); modified=true; }
        return modified;
      });
      if(cleared){ if(inst){ inst.fontColor=null; } return true; }
      focusTarget(target);
      let success=false;
      try{ document.execCommand("styleWithCSS", false, true); } catch(e){}
    ...
    function applyUnderline(inst, ctx){
      ...
      const handled=applyCommandToTableCells(inst, ctx, target, function(){
        const result=execCommand(target, "underline", null, true);
        if(style && selectionHasUnderline(target)){
          const decorated=applyDecorationStyle(inst, ctx, style, { skipTableSelection:true });
    ...
    function applyDecorationStyle(inst, ctx, style, opts){
      if(!style){ return false; }
      const target=resolveTarget(inst, ctx); if(!target) return false;
      if(!opts || !opts.skipTableSelection){
        const handled=applyCommandToTableCells(inst, ctx, target, function(){
          return applyDecorationStyle(inst, ctx, style, { skipTableSelection:true });
        });
      ...
      for(let i=0;i<updates.length;i++){
        ...
        if(hasUnderline){
          if(el.style.textDecorationLine!=="underline"){ el.style.textDecorationLine="underline"; changed=true; }
          if(el.style.textDecorationStyle!==style){ el.style.textDecorationStyle=style; changed=true; }
        }
      }
      return changed;
    }
    function applySimple(inst, ctx, command){
      const target=resolveTarget(inst, ctx); if(!target) return;
      const handled=applyCommandToTableCells(inst, ctx, target, function(){
        return execCommand(target, command, null, true);
      });
      if(handled){ return; }
    ...
    function applyAlign(inst, ctx, align){
      ...
      const targetAlign=normalized==="justify"?"justify":(normalized==="center"?"center":(normalized==="right"?"right":"left"));
      const styled=applyStyleToTableCells(inst, ctx, target, function(cell){
        if(cell.style.textAlign===targetAlign){ return false; }
        cell.style.textAlign=targetAlign;
        return true;
      });
    ...
    function applyLineSpacing(inst, ctx, spacing){
      ...
      const styled=applyStyleToTableCells(inst, ctx, target, function(cell){
        if(normalized){
          if(cell.style.lineHeight===normalized){ return false; }
      ...
      if(styled){ Normalizer.fixStructure(target); return true; }
      focusTarget(target);
      const doc=target.ownerDocument || document;
      const win=doc.defaultView || window;
      const sel=win.getSelection ? win.getSelection() : window.getSelection();
