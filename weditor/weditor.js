  ...
  const ImageTools=(function(){
    ...
    function attach(editor){
      ...
      if(editor.__weditorImageTools){
        ...
        if(overlay){
          const body=(editor.ownerDocument && editor.ownerDocument.body) ? editor.ownerDocument.body : document.body;
      ...
      function dispatchInput(){
        ...
        if(win && typeof win.InputEvent==="function"){
          try {
            ev=new win.InputEvent("input", { bubbles:true, cancelable:false, inputType:"insertReplacementText" });
          } catch(err){}
        }
        if(!ev){
          try {
            ...
            if(win && typeof win.Event==="function") ev=new win.Event("input", { bubbles:true });
            else ev=new Event("input", { bubbles:true });
          }
        }
      ...
      function updateOverlay(){
        ...
        if(!rect || rect.width<=0 || rect.height<=0){ hideOverlay(); return; }
        ...
        readout.textContent=formatReadout(width, height, hostRect.width||editor.clientWidth||width);
      }
      function scheduleOverlay(){
        if(raf){
          if(win && typeof win.cancelAnimationFrame==="function") win.cancelAnimationFrame(raf);
          else if(typeof window.cancelAnimationFrame==="function") window.cancelAnimationFrame(raf);
          else clearTimeout(raf);
          raf=null;
        }
        if(win && typeof win.requestAnimationFrame==="function") raf=win.requestAnimationFrame(updateOverlay);
        else if(typeof window.requestAnimationFrame==="function") raf=window.requestAnimationFrame(updateOverlay);
        else raf=window.setTimeout(updateOverlay, 16);
      }
      ...
      function startResize(spec, handleEl, ev){
        ...
        const rect=activeImg.getBoundingClientRect();
        let startWidth=Math.max(MIN_SIZE, rect && rect.width ? rect.width : 0);
        let startHeight=Math.max(MIN_SIZE, rect && rect.height ? rect.height : 0);
        if(startWidth<=MIN_SIZE && activeImg.naturalWidth){ startWidth=Math.max(MIN_SIZE, activeImg.naturalWidth); }
        if(startHeight<=MIN_SIZE && activeImg.naturalHeight){ startHeight=Math.max(MIN_SIZE, activeImg.naturalHeight); }
        if(!isFinite(startWidth) || startWidth<=0) startWidth=MIN_SIZE;
        if(!isFinite(startHeight) || startHeight<=0) startHeight=MIN_SIZE;
        const startX=ev.clientX;
        const startY=ev.clientY;
        const anchorX=spec.dir.indexOf("w")>=0 ? rect.right : rect.left;
        const anchorY=spec.dir.indexOf("n")>=0 ? rect.bottom : rect.top;
        const baseVectorX=spec.kind==="corner" ? (spec.dir.indexOf("w")>=0 ? -startWidth : startWidth) : 0;
        const baseVectorY=spec.kind==="corner" ? (spec.dir.indexOf("n")>=0 ? -startHeight : startHeight) : 0;
        const baseLengthSq=spec.kind==="corner" ? (baseVectorX*baseVectorX + baseVectorY*baseVectorY || 1) : 1;
        let changed=false;
        ...
        function onMove(moveEv){
          ...
          const pointerX=moveEv.clientX;
          const pointerY=moveEv.clientY;
          let width=startWidth;
          let height=startHeight;
          if(spec.kind==="corner"){
            let scale=( (pointerX-anchorX)*baseVectorX + (pointerY-anchorY)*baseVectorY )/baseLengthSq;
            ...
            width=startWidth+(pointerX-startX)*sign;
          } else if(spec.axis==="y"){
            const sign=spec.dir==="n"?-1:1;
            height=startHeight+(pointerY-startY)*sign;
          }
      ...
      const cleanup=function(){
        ...
        if(win && typeof win.removeEventListener==="function"){
          win.removeEventListener("resize", scheduleOverlay);
          win.removeEventListener("scroll", scheduleOverlay);
        } else {
