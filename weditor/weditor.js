  ...
  const TableResizer=(function(){
    ...
    function createState(inst, options){
      ...
      function ensureCellMetrics(cell){
        ...
        for(let i=0;i<nodes.length;i++){
          const node=nodes[i];
          if(node && node.nodeType===1){ firstEl=node; break; }
        }
        for(let i=nodes.length-1;i>=0;i--){
          const node=nodes[i];
          if(node && node.nodeType===1){ lastEl=node; break; }
        }
        let topInfo=null;
        let bottomInfo=null;
        if(firstEl){
          let computed=null;
          if(win && win.getComputedStyle){
            try { computed=win.getComputedStyle(firstEl); }
            catch(err){ computed=null; }
          }
          topInfo={
            el:firstEl,
            marginTop:computed?parseFloat(computed.marginTop)||0:0,
            inlineTop:(firstEl.style && typeof firstEl.style.marginTop==="string")?firstEl.style.marginTop:""
          };
          const inlineBottom=(firstEl.style && typeof firstEl.style.marginBottom==="string")?firstEl.style.marginBottom:"";
          const marginBottom=computed?parseFloat(computed.marginBottom)||0:0;
          topInfo.marginBottom=marginBottom;
          topInfo.inlineBottom=inlineBottom;
        }
        if(lastEl){
          if(topInfo && topInfo.el===lastEl){
            bottomInfo=topInfo;
          } else {
            let computed=null;
            if(win && win.getComputedStyle){
              try { computed=win.getComputedStyle(lastEl); }
              catch(err){ computed=null; }
            }
            bottomInfo={
              el:lastEl,
              marginBottom:computed?parseFloat(computed.marginBottom)||0:0,
              inlineBottom:(lastEl.style && typeof lastEl.style.marginBottom==="string")?lastEl.style.marginBottom:""
            };
          }
        }
        const metrics={ padTop, padBottom, borderTop, borderBottom, lineHeight, boxSizing, top:topInfo, bottom:bottomInfo };
      ...
      function applyChildMargins(metrics, top, bottom){
        if(!metrics) return;
        const safeTop=Math.max(0, Number.isFinite(top)?top:0);
        const safeBottom=Math.max(0, Number.isFinite(bottom)?bottom:0);
        if(metrics.top && metrics.top.el && metrics.top.marginTop!==undefined){
          metrics.top.el.style.marginTop=formatPixels(safeTop);
          if(metrics.top===metrics.bottom){
            metrics.top.el.style.marginBottom=formatPixels(safeBottom);
            return;
          }
        }
        if(metrics.bottom && metrics.bottom.el && metrics.bottom.marginBottom!==undefined){
          metrics.bottom.el.style.marginBottom=formatPixels(safeBottom);
        }
      }
      ...
      function restoreChildMargins(metrics){
        if(!metrics) return;
        if(metrics.top && metrics.top.el && metrics.top.marginTop!==undefined){
          if(metrics.top.inlineTop && metrics.top.inlineTop.length){
            metrics.top.el.style.marginTop=metrics.top.inlineTop;
          } else {
            metrics.top.el.style.marginTop=formatPixels(metrics.top.marginTop||0);
          }
          if(metrics.top===metrics.bottom){
            const bottomValue=(metrics.top.inlineBottom && metrics.top.inlineBottom.length)?metrics.top.inlineBottom:formatPixels(metrics.top.marginBottom||0);
            metrics.top.el.style.marginBottom=bottomValue;
            return;
          }
        }
        if(metrics.bottom && metrics.bottom.el && metrics.bottom.marginBottom!==undefined){
          if(metrics.bottom.inlineBottom && metrics.bottom.inlineBottom.length){
            metrics.bottom.el.style.marginBottom=metrics.bottom.inlineBottom;
          } else {
            metrics.bottom.el.style.marginBottom=formatPixels(metrics.bottom.marginBottom||0);
          }
        }
      }
      function setRowSize(row, value){
        ...
        for(let i=0;i<cells.length;i++){
          ...
          const baseMarginTop=(metrics && metrics.top && typeof metrics.top.marginTop==="number")?metrics.top.marginTop:0;
          let baseMarginBottom=0;
          if(metrics){
            if(metrics.bottom && typeof metrics.bottom.marginBottom==="number"){
              baseMarginBottom=metrics.bottom.marginBottom;
            } else if(metrics.top && metrics.top===metrics.bottom && typeof metrics.top.marginBottom==="number"){
              baseMarginBottom=metrics.top.marginBottom;
            }
          }
          const baseSpacingTotal=basePaddingTotal+baseMarginTop+baseMarginBottom;
          const spaceForContent=Math.max(0, finalValue - borderTotal);
          let padTop=basePadTop;
          let padBottom=basePadBottom;
          let marginTop=baseMarginTop;
          let marginBottom=baseMarginBottom;
          let lineHeightValue="";
          if(metrics && metrics.lineHeight){ lineHeightValue=metrics.lineHeight; }
          cell.style.boxSizing="border-box";
          cell.style.overflow="hidden";
          if(spaceForContent < baseSpacingTotal + MIN_HEIGHT){
            const adjustable=baseSpacingTotal;
            const available=Math.max(0, spaceForContent - MIN_HEIGHT);
            if(adjustable>0){
              const scale=available/adjustable;
              padTop=Math.floor(basePadTop*scale);
              padBottom=Math.floor(basePadBottom*scale);
              marginTop=Math.floor(baseMarginTop*scale);
              marginBottom=Math.floor(baseMarginBottom*scale);
              let used=padTop+padBottom+marginTop+marginBottom;
              let remainder=available-used;
              if(remainder>0){
                const adjustments=[
                  { key:"padTop", base:basePadTop, value:padTop },
                  { key:"padBottom", base:basePadBottom, value:padBottom },
                  { key:"marginTop", base:baseMarginTop, value:marginTop },
                  { key:"marginBottom", base:baseMarginBottom, value:marginBottom }
                ].filter(item=>item.base>item.value && item.base>0);
                adjustments.sort((a,b)=>b.base-a.base);
                for(let i=0;i<adjustments.length && remainder>0;i++){
                  const item=adjustments[i];
                  const capacity=Math.max(0, Math.round(item.base) - item.value);
                  if(!capacity) continue;
                  const add=Math.min(capacity, remainder);
                  if(add>0){
                    item.value+=add;
                    remainder-=add;
                  }
                ...
                for(let i=0;i<adjustments.length;i++){
                  const item=adjustments[i];
                  if(item.key==="padTop") padTop=item.value;
                  else if(item.key==="padBottom") padBottom=item.value;
                  else if(item.key==="marginTop") marginTop=item.value;
                  else if(item.key==="marginBottom") marginBottom=item.value;
                }
              }
            } else {
              padTop=0;
            ...
            const minimalContent=Math.min(MIN_HEIGHT, spaceForContent);
            ...
            applyChildMargins(metrics, marginTop, marginBottom);
          } else {
            cell.style.lineHeight=lineHeightValue;
            restoreCellPadding(cell, metrics);
            restoreChildMargins(metrics);
          }
