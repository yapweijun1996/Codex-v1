(function(){
    ...
    function paginate(rawHTML, state){
      ...
      function serializePages(){
        ...
        for(let i=0;i<pages.length;i++){
          ...
          if(pageEl && pageEl.__weditorAdaptiveHeight){
            if(hadHeight){ pageEl.style.height=originalHeight; }
            else { pageEl.style.removeProperty("height"); }
          }
        }
        return html;
      }
  ...
  const HistoryManager=(function(){
    ...
    function applyInstState(inst, state){
      ...
      if(Object.prototype.hasOwnProperty.call(state, "fixPageHeight")){
  ...
  function updateFixPageHeightState(inst){
    if(!inst || !inst.el) return;
    const fixed = inst.fixPageHeight!==false;
    inst.el.setAttribute("data-fix-page-height", fixed ? "true" : "false");
    inst.el.classList.toggle("weditor--adaptive-page", !fixed);
  }
  let INSTANCE_SEQ=0;
