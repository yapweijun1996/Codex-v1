  ...
  const TableResizer=(function(){
    ...
    function buildLayout(table){
      ...
      for(let r=0;r<rows.length;r++){
        ...
        if(grid[r] && grid[r].length>columnCount){ columnCount=grid[r].length; }
      }
    ...
    function measureColumn(table, colIndex, layout){
      const data=layout || buildLayout(table);
      ...
      for(let r=0;r<data.grid.length;r++){
        ...
        try {
          ...
          const span=metrics && Number.isFinite(metrics.colSpan) ? metrics.colSpan : parseSpan(cell.getAttribute("colspan")||cell.colSpan||"1",1);
          if(span>1){
            return Math.max(MIN_WIDTH, rect.width/span);
          }
          return rect.width;
        } catch(err){
          continue;
    ...
    function ensureTable(table){
      ...
      let expected=layout && layout.columnCount ? layout.columnCount : 0;
      if(expected<1){
        ...
        if(row && row.cells){
          for(let i=0;i<row.cells.length;i++){
            ...
            if(!cell) continue;
            expected+=parseSpan(cell.getAttribute("colspan")||cell.colSpan||"1",1);
          }
        }
      }
