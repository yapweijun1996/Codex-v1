(function(){
  ...
  const ImageTools=(function(){
    ...
    function attach(editor, options){
      ...
      if(editor.__weditorImageTools){
        ...
        if(onChange) existing.onChange=onChange;
        return existing;
      }
      ensureStyle();
      ...
      function dispatchInput(){
        ...
        const toolsRef=editor.__weditorImageTools;
        if(toolsRef && typeof toolsRef.onChange==="function"){
          try { toolsRef.onChange(); }
          catch(err){}
        }
      }
      ...
      function startResize(spec, handleEl, ev){
        ...
        const startRatio=(startWidth>0 && startHeight>0)?(startWidth/startHeight):null;
        const ratio=(startRatio && isFinite(startRatio) && startRatio>0)?startRatio:((naturalRatio && isFinite(naturalRatio) && naturalRatio>0)?naturalRatio:1);
      ...
      const tools={ overlay, hide:hideOverlay, select:selectImage, schedule:scheduleOverlay, cleanup, onChange:onChange };
  ...
  const HFEditor=(function(){
    function enableImageResizer(editor, onChange){
      if(!editor) return;
      const tools=ImageTools.attach(editor, onChange ? { onChange:onChange } : null);
    ...
    function section(kind, titleText, description, enabled, html, align){
      ...
      enableImageResizer(editor, notifyChange);
  ...
  const Fullscreen=(function(){
    function open(inst){
      ...
      ImageTools.attach(area, { onChange:function(){ render(); } });
  ...
  const ImageInsertUI=(function(){
    ...
    function insertImage(inst, ctx, src, alt){
      ...
      ImageTools.attach(target, { onChange:handleImageChange });
  ...
  WEditorInstance.prototype._mount=function(){
    ...
    ImageTools.attach(this.el, { onChange:function(){ OutputBinding.syncDebounced(self); } });
