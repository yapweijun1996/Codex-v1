  ...
  const HFEditor=(function(){
    ...
    function section(kind, titleText, description, enabled, html, align){
      ...
      for(let i=0;i<alignOptions.length;i++){
        ...
        btn.addEventListener("click", function(){ if(!toggle.checked) return; handleAlignRequest(opt.value); });
      ...
      function updateAlignButtonsState(activeValue){
      ...
      function updateAlignUI(){
        ...
        updateAlignButtonsState(alignValue);
      }
      ...
      function alignSelection(norm){
        ...
        if(!(ancestor===editor || editor.contains(ancestor))) return false;
        const command=norm==="center"?"justifyCenter":(norm==="right"?"justifyRight":"justifyLeft");
        const success=document.execCommand(command, false, null);
        if(success){ Normalizer.fixStructure(editor); }
        return success;
      }
      function handleAlignRequest(next){
        const norm=HFAlign.normalize(next);
        if(alignSelection(norm)){
          updateAlignButtonsState(norm);
        } else {
          setAlign(norm);
        }
      }
