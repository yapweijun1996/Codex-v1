(function(){
  ...
  const Formatting=(function(){
    ...
    function applyFontColor(inst, ctx, color, options){
      ...
      if(!skipFocus) focusTarget(target);
      let success=false;
      try{ document.execCommand("styleWithCSS", false, true); } catch(e){}
  ...
  const FontColorUI=(function(){
    ...
    function create(inst, ctx){
      ...
      let customPreview;
      let customInput;
      let customSessionOriginalColor=null;
      let customSelectionRange=null;
      let customSelectionOwner=null;
      function isRangeInsideTarget(range){
        if(!range) return false;
        const target=(ctx && ctx.area) ? ctx.area : inst ? inst.el : null;
        if(!target) return false;
        const container=range.commonAncestorContainer;
        if(!container) return false;
        if(container.nodeType===1 && target.contains(container)) return true;
        if(container.nodeType===3 && container.parentNode && target.contains(container.parentNode)) return true;
        return false;
      }
      function captureSelectionSnapshot(){
        ...
        customSelectionOwner=null;
        if(!sel || sel.rangeCount===0) return;
        try{
          const candidate=sel.getRangeAt(0);
          if(candidate && isRangeInsideTarget(candidate)){
            customSelectionRange=candidate.cloneRange();
            customSelectionOwner=sel;
          }
        } catch(err){
          customSelectionRange=null;
          customSelectionOwner=null;
      ...
      function restoreSelectionSnapshot(){
        if(!customSelectionOwner || !customSelectionRange) return;
        try{
          const clone=customSelectionRange.cloneRange ? customSelectionRange.cloneRange() : customSelectionRange;
          customSelectionOwner.removeAllRanges();
          customSelectionOwner.addRange(clone);
        } catch(err){
          customSelectionRange=null;
          customSelectionOwner=null;
        }
      ...
      function refreshSelectionSnapshot(){
        if(!customSelectionOwner) return;
        try{
          if(customSelectionOwner.rangeCount>0){
            const latest=customSelectionOwner.getRangeAt(0);
            if(latest && isRangeInsideTarget(latest)){
              customSelectionRange=latest.cloneRange();
              return;
            }
          }
        } catch(err){}
      ...
      moreBtn.addEventListener("click", function(e){
        e.preventDefault();
        e.stopPropagation();
        customSessionOriginalColor=typeof currentColor!=="undefined" ? currentColor : null;
      ...
      customInput.addEventListener("change", function(e){
        ...
        customSelectionRange=null;
        customSelectionOwner=null;
        customSessionOriginalColor=null;
        deactivateCustomPicker();
      });
      customInput.addEventListener("cancel", function(){
        const fallback=typeof customSessionOriginalColor!=="undefined" && customSessionOriginalColor!==null ? customSessionOriginalColor : null;
        if(fallback){
          Formatting.applyFontColor(inst, ctx, fallback);
        } else {
          Formatting.clearFontColor(inst, ctx);
        }
