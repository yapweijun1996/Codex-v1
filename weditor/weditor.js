(function(){
  ...
  const ImageTools=(function(){
    ...
    function attach(editor){
      ...
      function resolveInstance(){
        if(editor.__winst) return editor.__winst;
        let node=editor.parentNode;
        while(node){
          if(node.__winst) return node.__winst;
          node=node.parentNode;
        }
        return null;
      }
      function syncOutputs(){
        if(typeof OutputBinding==='undefined') return;
        const inst=resolveInstance();
        if(inst) OutputBinding.syncDebounced(inst);
      }
      function recordHistory(label){
        if(typeof HistoryManager==='undefined' || typeof OutputBinding==='undefined') return;
        const inst=resolveInstance();
        if(!inst) return;
        HistoryManager.record(inst, editor, { label:label||"Edit Image", repeatable:false });
      ...
      function onInput(){ if(activeImg && !editor.contains(activeImg)) hideOverlay(); scheduleOverlay(); syncOutputs(); }
      ...
      function startResize(spec, handleEl, ev){
        ...
        function applySize(width, height){
          ...
          if(Math.abs(nextWidth-lastWidth)>=0.5 || Math.abs(nextHeight-lastHeight)>=0.5){
            changed=true;
            lastWidth=nextWidth;
  ...
  const ImageInsertUI=(function(){
    ...
    function insertImage(inst, ctx, src, alt){
      ...
      function calibrateInitialSize(){
        if(!img || img.__weditorSized) return;
        const naturalWidth=Math.max(0, img.naturalWidth||0);
        const naturalHeight=Math.max(0, img.naturalHeight||0);
        if(!naturalWidth || !naturalHeight) return;
        const ratio=naturalHeight/naturalWidth || 1;
        const hostRect=target && target.getBoundingClientRect ? target.getBoundingClientRect() : null;
        let available=Math.max(0, target ? target.clientWidth : 0);
        if((!available || !isFinite(available)) && hostRect){ available=Math.max(0, hostRect.width); }
        if(!available || !isFinite(available) || available<=0){ available=naturalWidth; }
        let width=Math.min(naturalWidth, available);
        if(!isFinite(width) || width<=0) width=naturalWidth;
        const height=width*ratio;
        img.style.maxWidth="100%";
        img.style.width=Math.round(width)+"px";
        img.style.height=Math.round(height)+"px";
        if(!img.style.objectFit) img.style.objectFit="contain";
        img.__weditorSized=true;
        if(inst) OutputBinding.syncDebounced(inst);
      }
      if(img.complete && img.naturalWidth && img.naturalHeight){
        calibrateInitialSize();
      } else {
        img.addEventListener("load", function(){ calibrateInitialSize(); }, { once:true });
      }
