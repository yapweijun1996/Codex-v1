(function(){
  ...
  const TableResizer=(function(){
    ...
    function columnCell(table, colIndex, layout){
      ...
      if(layout && layout.grid){
        for(let r=0;r<layout.grid.length;r++){
          ...
          if(layout.cells){
            ...
            if(metrics && metrics.colSpan===1) return cell;
      ...
      for(let r=0;r<rows.length;r++){
        ...
        if(!cell || !table.contains(cell)) continue;
        if((cell.colSpan||1)===1) return cell;
      }
      if(layout && layout.grid){
        for(let r=0;r<layout.grid.length;r++){
          const rowCells=layout.grid[r];
          if(!rowCells) continue;
          const cell=rowCells[colIndex];
          if(cell && table.contains(cell)) return cell;
        }
      }
      return null;
    }
    ...
    function measureColumn(table, colIndex, layout){
      const cell=columnCell(table, colIndex, layout);
      if(!cell) return 0;
      const rect=cell.getBoundingClientRect();
      if(!rect || !rect.width) return 0;
      if(layout && layout.cells){
        const metrics=layout.cells.get(cell);
        if(metrics && metrics.colSpan>1){
          const span=Math.max(1, metrics.colSpan||1);
    ...
    function ensureTable(table){
      ...
      let layout=null;
      let expected=0;
      if(typeof TableMatrix!=="undefined" && TableMatrix && typeof TableMatrix.read==="function"){
        layout=TableMatrix.read(table);
        if(layout && layout.grid){
          expected=layout.grid.reduce(function(max, row){ return row ? Math.max(max, row.length) : max; }, 0);
        }
      }
      if(!expected){
        const row=firstBodyRow(table);
        if(!row) return null;
        expected=row.cells.length;
    ...
    function createState(inst, options){
        ...
        if(withinColumn){
          ...
          if(colgroup){
            ...
            if(colCount>0){
              ...
              if(typeof TableMatrix!=="undefined" && TableMatrix && typeof TableMatrix.getCellMetrics==="function"){
                const metrics=TableMatrix.getCellMetrics(table, cell);
                if(metrics){
                  cellIndex=metrics.colIndex+metrics.colSpan-1;
                }
              }
      ...
      function readColumnWidth(table, colgroup, index){
        ...
        return measureColumn(table, index, layout);
      }
