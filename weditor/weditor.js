  ...
  const Breaks=(function(){
    ...
    function remove(targetEl){
      ...
      if(!node || !targetEl.contains(node)){
        for(let i=targetEl.childNodes.length-1;i>=0;i--){ const n=targetEl.childNodes[i]; if(isBreakComment(n)){ return removeCommentNode(targetEl, n, "backward"); } }
        alert("No page break found near cursor."); return false;
      }
      while(node && node.parentNode!==targetEl){ node=node.parentNode; }
      let f=node.nextSibling; while(isWhitespace(f)) f=f.nextSibling; if(isBreakComment(f)){ return removeCommentNode(targetEl, f, "forward"); }
      let b=node.previousSibling; while(isWhitespace(b)) b=b.previousSibling; if(isBreakComment(b)){ return removeCommentNode(targetEl, b, "backward"); }
