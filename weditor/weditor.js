(function(){
  ...
  const TableResizer=(function(){
    ...
    function buildLayout(table){
      ...
      for(let r=0;r<rows.length;r++){
        ...
        for(let c=0;c<row.cells.length;c++){
          ...
          const rowSpan=parseSpan(cell.getAttribute("rowspan")||cell.rowSpan||"1");
          const colSpan=parseSpan(cell.getAttribute("colspan")||cell.colSpan||"1");
    ...
    function measureColumn(table, layout, colIndex, cache){
      if(!table || !layout || !layout.grid) return 0;
      ...
      for(let r=0;r<grid.length;r++){
        ...
        const info=layout.info.get(cell);
        const span=info && info.colSpan ? info.colSpan : 1;
        let width=cache.has(cell) ? cache.get(cell) : null;
        if(width==null){
          const rect=cell.getBoundingClientRect();
          width=rect && rect.width ? rect.width : 0;
          cache.set(cell, width);
        }
        if(width<=0) continue;
        if(span<=1) return width;
        const perColumn=width/span;
        if(perColumn>fallback) fallback=perColumn;
      }
      return fallback;
    }
    function ensureWidths(table, colgroup, layout){
      ...
      for(let i=0;i<cols.length;i++){
        const col=cols[i];
        const measured=measureColumn(table, layout, i, cache);
        const widthValue=measured>0 ? measured : fallbackWidth;
        col.style.width=Math.max(MIN_WIDTH, Math.round(widthValue))+"px";
      }
    }
    function ensureTable(table){
      ...
      for(let i=0;i<layout.grid.length;i++){
        ...
        if(row && row.length>expected) expected=row.length;
      }
