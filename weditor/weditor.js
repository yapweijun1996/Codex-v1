(function(){
  ...
  const TableSelection=(function(){
    ...
    function addCellToSelection(state, table, cell){
      if(!state || !cell || !table) return;
      if(state.table && state.table!==table){
        clearSelection(state);
      }
      ensureStyles(table.ownerDocument || document);
      if(cell.classList){ cell.classList.add(CELL_CLASS); }
    ...
    function deleteSelection(state){
      ...
      const connectedCells=Array.from(state.cells).filter(function(cell){ return cell && cell.isConnected && table.contains(cell); });
      ...
      for(let i=0;i<connectedCells.length;i++){
        const row=closestRow(connectedCells[i]);
        if(row && row.isConnected && table.contains(row)){
          rowSet.add(row);
        }
      }
      const selectedRows=Array.from(rowSet);
    ...
    function handleMouseDown(state, event){
      ...
      if(event.shiftKey){
        if(!cell){ return; }
        const anchorCell=isValidAnchor(state) || (state.cells && state.cells.size ? state.cells.values().next().value : null);
        const targetTable=tableFromCell(cell);
        if(!anchorCell){
          state.anchor=cell;
          clearSelection(state);
          addCellToSelection(state, targetTable, cell);
          return;
        }
        ...
        event.preventDefault();
        if(!state.cells.has(anchorCell)){
          addCellToSelection(state, anchorTable, anchorCell);
        }
        addCellToSelection(state, anchorTable, cell);
        focusCell(cell);
        state.anchor=cell;
        return;
      }
      if(cell){
        state.anchor=cell;
        clearSelection(state);
        addCellToSelection(state, tableFromCell(cell), cell);
      } else {
        state.anchor=null;
        clearSelection(state);
      }
