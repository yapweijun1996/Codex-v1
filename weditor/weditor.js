(function(){
  ...
  const ImageTools=(function(){
    ...
    function attach(editor){
      ...
      function queueResizeDispatch(){
        if(resizeInputRaf) return;
        resizeInputRaf=requestAnimationFrame(function(){
          resizeInputRaf=null;
          dispatchInput();
        });
      }
      function startResize(spec, handleEl, ev){
        ...
        let ratio=(startWidth && startHeight)?(startWidth/startHeight):0;
        if((!ratio || !isFinite(ratio)) && activeImg.naturalWidth && activeImg.naturalHeight){
          ratio=activeImg.naturalWidth/activeImg.naturalHeight;
        }
        ...
        function applySize(width, height){
          ...
          const prevWidthPx=Math.round(prevWidth);
          const prevHeightPx=Math.round(prevHeight);
          const dimensionChanged=(prevWidthPx!==widthPx || prevHeightPx!==heightPx);
          if(!dimensionChanged){ scheduleOverlay(); return; }
        ...
        function onMove(moveEv){
          ...
          if(spec.kind==="corner"){
            ...
            let scale;
            if(!isFinite(widthScale) || widthScale<=0){ scale=heightScale; }
            else if(!isFinite(heightScale) || heightScale<=0){ scale=widthScale; }
            else {
              const absWidth=Math.abs(widthScale-1);
              const absHeight=Math.abs(heightScale-1);
              scale = absWidth>absHeight ? widthScale : heightScale;
            }
