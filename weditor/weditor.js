      ...
      if(!tableRemoved){
        ...
        if(layout && layout.cells){
          ...
          layout.cells.forEach(function(metrics, cell){
            ...
            const isSelected=state.cells.has(cell);
            for(let rr=0; rr<metrics.rowSpan; rr++){
              const rowIndex=metrics.rowIndex+rr;
              for(let cc=0; cc<metrics.colSpan; cc++){
                const colIndex=metrics.colIndex+cc;
                if(!columnInfo.has(colIndex)){
                  columnInfo.set(colIndex, { rows:new Set(), selectedRows:new Set() });
                }
                const info=columnInfo.get(colIndex);
                info.rows.add(rowIndex);
                if(isSelected) info.selectedRows.add(rowIndex);
              }
            }
          });
          const columnsToDelete=[];
          columnInfo.forEach(function(info, index){
            const total=info.rows.size;
            const selected=info.selectedRows.size;
          ...
          if(columnsToDelete.length){
            ...
            const columnSet=new Set(columnsToDelete);
            const processed=new Set();
            layout.cells.forEach(function(metrics, cell){
              if(!cell || !cell.isConnected || processed.has(cell)) return;
              const row=closestRow(cell);
              if(!row || rowsMarked.has(row)) return;
              let removeCount=0;
              for(let cc=0; cc<metrics.colSpan; cc++){
                if(columnSet.has(metrics.colIndex+cc)) removeCount++;
              }
              ...
              if(removeCount>=metrics.colSpan){
                ...
                if(!columnFocusTarget){
                  columnFocusTarget=findColumnFocusCandidate(table, row, metrics.rowIndex, metrics.colIndex);
                }
                if(row && (!row.cells || !row.cells.length) && !rowsMarked.has(row)){
                  rowsMarked.add(row);
                  if(row.parentNode){ row.parentNode.removeChild(row); additionalRowsRemoved++; }
                }
              } else {
                const newSpan=Math.max(1, metrics.colSpan-removeCount);
