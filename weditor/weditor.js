(function(){
  ...
  const TemplateRegistry=(function(){
    ...
    function normalizeAlign(value){
      return HFAlign.normalize(value || "left");
    }
    function resolvePreview(preview, summary, label, body){
      const first = preview && preview.trim();
      if(first) return first;
      const second = summary && summary.trim();
      if(second) return second;
      const third = body && body.trim();
      if(third) return third;
      return label || "Template";
    }
    function toTemplate(el, index){
      if(!el || el.tagName!=="TEXTAREA") return null;
      const raw=readValue(el);
      if(!raw) return null;
      const state=StateBinding.parse(raw);
      if(!state || typeof state!=="object") return null;
      const labelAttr=el.getAttribute("data-weditor-template-label");
      const summaryAttr=el.getAttribute("data-weditor-template-summary") || el.getAttribute("title");
      const previewHTMLAttr=el.getAttribute("data-weditor-template-preview-html") || el.getAttribute("data-weditor-template-preview");
      const previewTextAttr=el.getAttribute("data-weditor-template-preview-text");
      const targetAttr=el.getAttribute("data-weditor-template-for");
      let label=(labelAttr && labelAttr.trim()) || (state.label && String(state.label).trim()) || "Template "+(index+1);
      if(!label) label="Template "+(index+1);
      let idAttr=el.getAttribute("data-weditor-template-id") || state.id;
      if(idAttr) idAttr=String(idAttr).trim();
      const id=idAttr || (slugify(label)+"-"+(index+1));
      const summary=(summaryAttr && summaryAttr.trim()) || (state.summary && String(state.summary).trim()) || "";
      let preview=(previewHTMLAttr && previewHTMLAttr.trim()) || "";
      if(!preview && previewTextAttr && previewTextAttr.trim()) preview=previewTextAttr.trim();
      const body=typeof state.html==="string" ? state.html : "";
      const template={
        id:id,
        label:label,
        summary:summary,
        preview:resolvePreview(preview, summary, label, body),
        body:body,
        header:{},
        footer:{},
        state:state,
        outputMode:state.outputMode,
        fixPageHeight:Object.prototype.hasOwnProperty.call(state, "fixPageHeight") ? state.fixPageHeight : undefined,
        targetId:targetAttr && targetAttr.trim() ? targetAttr.trim() : null
      };
      const header=state.header && typeof state.header==="object" ? state.header : {};
      const footer=state.footer && typeof state.footer==="object" ? state.footer : {};
      const headerHTML=typeof header.html==="string" ? header.html : (typeof state.headerHTML==="string" ? state.headerHTML : "");
      const footerHTML=typeof footer.html==="string" ? footer.html : (typeof state.footerHTML==="string" ? state.footerHTML : "");
      let headerEnabled;
      if(typeof header.enabled==="boolean") headerEnabled=header.enabled;
      else if(typeof state.headerEnabled==="boolean") headerEnabled=state.headerEnabled;
      let footerEnabled;
      if(typeof footer.enabled==="boolean") footerEnabled=footer.enabled;
      else if(typeof state.footerEnabled==="boolean") footerEnabled=state.footerEnabled;
      template.header={
        html:headerHTML,
        align:normalizeAlign(header.align || state.headerAlign),
        enabled:headerEnabled
      };
      template.footer={
        html:footerHTML,
        align:normalizeAlign(footer.align || state.footerAlign),
    ...
    function collectNodes(doc){
      if(!doc) return [];
      return doc.querySelectorAll("textarea.weditor_template_json, textarea[data-weditor-template-json], textarea[data-weditor-template]");
    }
    function list(inst){
      const doc=(inst && inst.el && (inst.el.ownerDocument || document)) || document;
      const nodes=collectNodes(doc);
      const editorId=inst && inst.el && inst.el.getAttribute && inst.el.getAttribute("id");
      const templates=[];
      for(let i=0;i<nodes.length;i++){
        const tpl=toTemplate(nodes[i], i);
        if(!tpl) continue;
        if(tpl.targetId && tpl.targetId!==editorId) continue;
        templates.push(tpl);
      }
      return templates;
    }
    function listAll(doc){
      const nodes=collectNodes(doc || document);
      const templates=[];
      for(let i=0;i<nodes.length;i++){
        const tpl=toTemplate(nodes[i], i);
  ...
  const DocumentTemplateUI=(function(){
    ...
    function resolveBlock(tpl, key, inst){
      const fallback={ html:"", align:HFAlign.normalize("left"), enabled:undefined };
      ...
      const meta=tpl[key];
      if(meta){
        const raw=resolveValue(meta.html, inst);
        if(raw) html=raw;
        if(meta.align) align=meta.align;
        if(typeof meta.enabled==="boolean") enabled=meta.enabled;
      }
      const state=tpl.state;
      if(state && typeof state==="object"){
        const block=state[key] && typeof state[key]==="object" ? state[key] : {};
        if(typeof block.html==="string") html=block.html;
        else {
          const legacyKey=key+"HTML";
          if(typeof state[legacyKey]==="string") html=state[legacyKey];
        }
        if(block.align) align=block.align;
        else {
          const legacyAlignKey=key+"Align";
          if(state[legacyAlignKey]) align=state[legacyAlignKey];
        }
        if(typeof block.enabled==="boolean") enabled=block.enabled;
        else {
          const legacyEnabledKey=key+"Enabled";
          if(typeof state[legacyEnabledKey]==="boolean") enabled=state[legacyEnabledKey];
        }
      }
      return {
        html:html || "",
        align:HFAlign.normalize(align || "left"),
        enabled:enabled
      };
    }
    ...
    function applyTemplate(inst, template){
      ...
      if(template.state){
        const cloned=cloneState(template.state);
        if(!cloned) return false;
        const applied=StateBinding.apply(inst, cloned);
    ...
    function open(inst){
      ...
      function updatePreview(tpl){
        ...
        const headerMeta=resolveBlock(tpl, "header", inst);
        const footerMeta=resolveBlock(tpl, "footer", inst);
        const headerHTML=headerMeta.html ? replaceTokens(headerMeta.html) : "";
        const footerHTML=footerMeta.html ? replaceTokens(footerMeta.html) : "";
        const showHeader=headerMeta.enabled!==false && headerHTML && headerHTML.trim();
        const showFooter=footerMeta.enabled!==false && footerHTML && footerHTML.trim();
        const bodyPreview=tpl.preview ? resolveValue(tpl.preview, inst) : resolveValue(tpl.body, inst);
        previewHeader.innerHTML=Sanitizer.clean(showHeader ? headerHTML : '<span style="font-size:12px;color:#666;">Header disabled</span>');
        previewBody.innerHTML=Sanitizer.clean(replaceTokens(bodyPreview || ""));
        previewFooter.innerHTML=Sanitizer.clean(showFooter ? footerHTML : '<span style="font-size:12px;color:#666;">Footer disabled</span>');
        detailSummary.textContent=resolveValue(tpl.summary || "", inst);
        setApplyState(true);
      }
      const templates=TemplateRegistry.list(inst);
      for(let i=0;i<templates.length;i++){
        const tpl=templates[i];
      ...
      if(!templates.length){
        intro.textContent="No templates detected. Add <textarea class=\"weditor_template_json\"> blocks with stored JSON to offer templates.";
