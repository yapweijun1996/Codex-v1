        ...
        if(layout && layout.grid && layout.grid.length){
          ...
          for(let col=0;col<maxColumns;col++){
            ...
            for(let r=0;r<layout.grid.length;r++){
              ...
              if(!row) continue;
              const cell=row[col];
              if(!cell || !isCellElement(cell)) continue;
              const rowEl=closestRow(cell);
              if(!rowEl || rowsMarked.has(rowEl)) continue;
              total++;
              cellSet.add(cell);
              if(state.cells.has(cell)) selectedCount++;
            }
            if(total>0 && selectedCount>=total){
              columnCells.set(col, cellSet);
            }
          }
          if(columnCells.size){
            const sortedColumns=Array.from(columnCells.keys()).sort(function(a,b){ return a-b; });
            const processedCells=new Set();
            const metricsMap=layout.cells;
            sortedColumns.forEach(function(colIndex){
              const cells=columnCells.get(colIndex);
              if(!cells) return;
              cells.forEach(function(cell){
                if(!cell || processedCells.has(cell)) return;
                const metrics=metricsMap.get(cell);
                if(!metrics) return;
                const rowEl=closestRow(cell);
                if(!rowEl || rowsMarked.has(rowEl)) return;
                let removeCount=0;
                for(let i=0;i<sortedColumns.length;i++){
                  const idx=sortedColumns[i];
                  if(idx>=metrics.colIndex && idx<metrics.colIndex+metrics.colSpan){ removeCount++; }
                }
                ...
                if(removeCount>=metrics.colSpan){
                  ...
                  if(parent){
                    ...
                    columnsRemoved=true;
                    const rowIndex=typeof rowEl.rowIndex==="number" ? rowEl.rowIndex : (table.rows ? Array.prototype.indexOf.call(table.rows, rowEl) : -1);
                    if(!columnFocusTarget){
                      columnFocusTarget=findColumnFocusCandidate(table, rowEl, rowIndex, colIndex);
                    }
                    if(!rowEl.cells || !rowEl.cells.length){
                      rowsMarked.add(rowEl);
                      if(rowEl.parentNode){ rowEl.parentNode.removeChild(rowEl); additionalRowsRemoved++; }
                    }
                  }
                } else {
                  const newSpan=metrics.colSpan-removeCount;
                  cell.colSpan=newSpan;
                  if(newSpan<=1 && cell.hasAttribute("colspan")){
                    cell.removeAttribute("colspan");
                  }
                  columnsRemoved=true;
                  if(!columnFocusTarget){ columnFocusTarget=cell; }
                }
              });
            });
            if(columnsRemoved){
              ...
              if(!focusTarget && !columnFocusTarget){
                const refreshed=TableMatrix.read(table);
                if(refreshed && refreshed.grid){
                  outerFocus:
                  for(let r=0;r<refreshed.grid.length;r++){
                    const row=refreshed.grid[r];
                    if(!row) continue;
                    for(let c=0;c<row.length;c++){
                      const candidate=row[c];
                      if(candidate && isCellElement(candidate) && candidate.isConnected && table.contains(candidate)){
