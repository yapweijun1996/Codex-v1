(function(){
  ...
  const ImageTools=(function(){
    ...
    function captureDimensions(img){
      ...
      if(width>0){ img.setAttribute("data-weditor-width", String(width)); }
      if(height>0){ img.setAttribute("data-weditor-height", String(height)); }
    }
    ...
    function attach(editor){
      ...
      overlay.style.outline="1px dashed "+WCfg.UI.brand;
      ...
      function ensureImageWatcher(img){
        if(!img) return;
        if(!img.__weditorSizeHandler){
          const handler=function(){ captureDimensions(img); scheduleOverlay(); scheduleDispatch(); };
        ...
        if(typeof window!=="undefined" && window.ResizeObserver && !img.__weditorResizeObserver){
          const observer=new window.ResizeObserver(function(){ captureDimensions(img); scheduleOverlay(); scheduleDispatch(); });
          observer.observe(img);
          img.__weditorResizeObserver=observer;
        }
      }
      ...
      function detachImageWatchers(){
        ...
        for(let i=0;i<imgs.length;i++){
          ...
          if(img.__weditorResizeObserver){
            try { img.__weditorResizeObserver.disconnect(); }
            catch(err){}
            delete img.__weditorResizeObserver;
          }
        }
      }
      let dispatchTimer=null;
      function dispatchInput(){
        if(dispatchTimer && typeof window!=="undefined"){ window.clearTimeout(dispatchTimer); dispatchTimer=null; }
        if(!editor) return;
        let ev=null;
        if(typeof window!=="undefined" && typeof window.InputEvent==="function"){
          try {
            ev=new window.InputEvent("input", { bubbles:true, cancelable:false });
          } catch(err){ ev=null; }
        }
        if(!ev){ ev=new Event("input", { bubbles:true }); }
        editor.dispatchEvent(ev);
        const changeEv=new Event("change", { bubbles:true });
        editor.dispatchEvent(changeEv);
      }
      function scheduleDispatch(){
        if(dispatchTimer || typeof window==="undefined") return;
        dispatchTimer=window.setTimeout(function(){
          dispatchTimer=null;
          dispatchInput();
        }, 120);
      }
      ...
      const cleanup=function(){
        if(dispatchTimer){ window.clearTimeout(dispatchTimer); dispatchTimer=null; }
