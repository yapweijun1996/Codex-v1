  ...
  const TableResizer=(function(){
    ...
    function createState(inst, options){
      ...
      function ensureCellMetrics(cell){
        ...
        const metrics={ padTop, padBottom, borderTop, borderBottom, lineHeight, boxSizing, children:childrenMetrics };
      ...
      function restoreChildMargins(cell, metrics){
        ...
        for(let i=0;i<list.length;i++){
          ...
          if(info.lineHeight){ el.style.lineHeight=info.lineHeight; }
        }
      }
      function adjustChildMargins(cell, metrics, allowance){
        ...
        for(let i=0;i<list.length;i++){
          ...
          applied.push({ el, top, bottom, weight:baseTop+baseBottom });
          used+=top+bottom;
        }
        if(applied.length>0 && Math.abs(used-safeAllowance)>0.01){
          const diff=safeAllowance-used;
          const target=applied.reduce((best, item)=>{
            if(!best) return item;
            const bestWeight=Number.isFinite(best.weight)?best.weight:0;
            const itemWeight=Number.isFinite(item.weight)?item.weight:0;
            return itemWeight>bestWeight ? item : best;
          }, null);
          if(target){
            if(target.bottom>=target.top){ target.bottom=Math.max(0, target.bottom+diff); }
            else { target.top=Math.max(0, target.top+diff); }
            used+=diff;
          }
        }
        let finalUsed=0;
        for(let i=0;i<applied.length;i++){
          const info=applied[i];
          const el=info.el;
          el.style.marginTop=formatPixels(info.top);
          el.style.marginBottom=formatPixels(info.bottom);
          finalUsed+=Math.max(0, info.top)+Math.max(0, info.bottom);
        }
        return Math.min(finalUsed, safeAllowance);
      }
          ...
          if(spaceForContent < basePaddingTotal + MIN_HEIGHT){
            ...
            if(basePaddingTotal>0){
              ...
              if(Math.abs(used-availableForPadding)>0.01){
                const diff=availableForPadding-used;
                if(basePadTop>=basePadBottom){ padTop=Math.max(0, padTop+diff); }
                else { padBottom=Math.max(0, padBottom+diff); }
            ...
            const remainingAfterPadding=Math.max(0, spaceForContent - (padTop+padBottom));
            const marginAllowance=Math.max(0, remainingAfterPadding - MIN_HEIGHT);
            const marginUsed=adjustChildMargins(cell, metrics, marginAllowance);
            const remainingAfterMargins=Math.max(0, remainingAfterPadding - marginUsed);
            const contentHeight=Math.max(MIN_HEIGHT, remainingAfterMargins);
