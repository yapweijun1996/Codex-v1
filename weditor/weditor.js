    ...
    function paginate(rawHTML, state){
      ...
      function ensureMeasuredBlock(block, containerWidth){
        ...
        for(let k=0;k<imgs.length;k++){
          ...
          const inlineWidth=(style.width||"").trim();
          const inlineHeight=(style.height||"").trim();
          const attrWidth=(img.getAttribute?img.getAttribute("width"):null)||"";
          const attrHeight=(img.getAttribute?img.getAttribute("height"):null)||"";
          const normalizeAttr=function(value){
          ...
          if(!inlineWidth){
            const normalized=normalizeAttr(attrWidth);
            if(normalized) style.width=normalized;
          }
          if(!inlineHeight){
            const normalizedH=normalizeAttr(attrHeight);
            if(normalizedH) style.height=normalizedH;
          }
          const normalizeFlag=function(value){
            const trimmed=(value||"").trim().toLowerCase();
            if(!trimmed) return "";
            if(trimmed==="auto" || trimmed==="initial" || trimmed==="inherit" || trimmed==="unset" || trimmed==="revert" || trimmed==="revert-layer") return "";
            return trimmed;
          };
          const hasExplicitWidth=!!normalizeFlag(style.width||"");
          const hasExplicitHeight=!!normalizeFlag(style.height||"");
          if(hasExplicitWidth || hasExplicitHeight){
            if(!hasExplicitWidth){ style.maxWidth="100%"; }
            else { style.maxWidth=""; }
            if(hasExplicitHeight){ style.maxHeight=style.height; }
            else { style.maxHeight=""; style.height="auto"; }
            continue;
          }
          style.maxWidth="100%";
          if(img.naturalWidth && img.naturalHeight){
            const scale=containerWidth && img.naturalWidth ? Math.min(1, containerWidth / img.naturalWidth) : 1;
            const expected=Math.round(img.naturalHeight * scale);
            style.height=expected+"px";
            style.maxHeight=expected+"px";
          } else {
            style.height="auto";
            style.maxHeight="";
          }
  ...
  const ImageTools=(function(){
    ...
    function attach(editor){
      ...
      function notifyChange(){
        const inst=editor && editor.__winst;
      ...
      function onMovePointerDown(ev){
        ...
        function finish(endEv){
          ...
          if(!dropRange && endEv){
            const fallback=getValidDropRange(endEv.clientX, endEv.clientY);
            if(fallback) dropRange=fallback;
          }
          const sel=getSelection();
          if(dropRange && editor.contains(dropRange.startContainer)){
            if(moveImageToRange(activeImg, dropRange)) notifyChange();
